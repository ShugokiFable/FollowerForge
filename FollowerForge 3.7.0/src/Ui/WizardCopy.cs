...
public static class WizardCopy
{
    ...
    public static string TransformHint(FollowerPronouns p) =>
        p.Fill("{Subject} transforms when a fight starts and changes back when it ends. Werewolf uses the game's own beast race and needs nothing installed. Custom takes any race and/or spell you already have — creature races included, so a dragon, troll or wolf form is a valid choice. Worn gear usually stops showing while {subject} is a creature, and comes back on revert. Also adds a script.");
