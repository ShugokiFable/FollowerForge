...
public static class FollowerValidator
{
    ...
    private static void ValidateMod(ISkyrimModGetter mod, FollowerProfile profile, ValidationReport report)
    {
        ...
        // The compiler takes ONE of two branches: a legacy user-chosen OTFT wins outright, and
        // only otherwise is a private StartingEquipment outfit generated from the picked armor.
        // This check has to follow the same branch. It used to run the generated-outfit check
        // unconditionally, so choosing an outfit AND any armor searched THIS plugin for an outfit
        // that lives in Skyrim.esm or a mod, found nothing, and failed the build once per piece.
        if (profile.Outfit is { } chosenOutfit)
        {
            var expected = Mutagen.Bethesda.Plugins.FormKey.Factory(chosenOutfit.FormKey);
            if (npc.DefaultOutfit.FormKey != expected)
                report.Add(ValidationSeverity.Error, "OUTFIT_NOT_ASSIGNED",
                    $"Chosen outfit {expected} was not assigned to the NPC "
                    + $"(DefaultOutfit is {npc.DefaultOutfit.FormKey}).");
        }
