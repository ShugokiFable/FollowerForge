...
public static class Mo2PathResolver
{
    ...
    /// <summary>
    /// True when the plugin is available to the active manager the same way the game would see it.
    /// Vortex: deployed game Data. MO2: enabled mod folders / overwrite, then game Data.
    /// </summary>
    public static bool PluginIsInstalled(EnvironmentSnapshot env, string pluginFileName) =>
        env.Manager == ModManagerKind.Mo2
            ? ResolvePlugin(env, pluginFileName) is not null
