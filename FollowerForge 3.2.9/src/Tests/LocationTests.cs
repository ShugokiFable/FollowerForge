...
public sealed class LocationTests : IDisposable
{
    ...
    [Fact]
    public void Compile_ManualCellPlacement_DoesNotHardcodeWhiterun()
    {
        // Manual Cell + XYZ without LocationId must place into the specified interior cell,
        // not WhiterunWorld's persistent cell (the 3.2.7/3.2.8 bug).
        var profile = new FollowerProfile
        {
            Name = "Manual Cell",
            PluginName = "FF_ManualCell.esp",
            Race = new RecordRef(VanillaForms.NordRace.ToString()),
            VoiceType = new RecordRef(VanillaForms.FemaleEvenTonedVoice.ToString()),
            Class = new RecordRef(VanillaForms.CombatWarrior1HClass.ToString()),
            Placement = new PlacementSpec
            {
                Cell = new RecordRef("01605E:Skyrim.esm"),
                X = 12, Y = 34, Z = 56,
            },
        };
        ...
        Assert.Empty(result.Mod.Worldspaces);
        var cell = result.Mod.Cells.Records
            .SelectMany(b => b.SubBlocks)
            .SelectMany(s => s.Cells)
            .Single();
        Assert.Equal(FormKey.Factory("01605E:Skyrim.esm"), cell.FormKey);
        Assert.Equal(44, cell.FormVersion);
        var placed = Assert.Single(cell.Persistent.OfType<IPlacedNpcGetter>());
        Assert.Equal(12, placed.Placement!.Position.X);
        Assert.Equal(34, placed.Placement.Position.Y);
        Assert.Equal(56, placed.Placement.Position.Z);
    }

    [Fact]
    public void Compile_ManualExteriorWithWorldspace_UsesThatWorldspace()
    {
        var solstheim = FormKey.Factory("000800:Dragonborn.esm");
        var solstheimCell = FormKey.Factory("000801:Dragonborn.esm");
        var profile = new FollowerProfile
        {
            Name = "Solstheim Spot",
            PluginName = "FF_Solstheim.esp",
            Race = new RecordRef(VanillaForms.NordRace.ToString()),
            VoiceType = new RecordRef(VanillaForms.FemaleEvenTonedVoice.ToString()),
            Class = new RecordRef(VanillaForms.CombatWarrior1HClass.ToString()),
            Placement = new PlacementSpec
            {
                Cell = new RecordRef(solstheimCell.ToString()),
                Worldspace = new RecordRef(solstheim.ToString()),
                X = 1000, Y = 2000, Z = 300,
            },
        };
        ...
        var ws = Assert.Single(result.Mod.Worldspaces);
        Assert.Equal(solstheim, ws.FormKey);
        Assert.NotNull(ws.TopCell);
        Assert.Equal(solstheimCell, ws.TopCell!.FormKey);
        Assert.Equal(44, ws.TopCell.FormVersion);
        var placed = Assert.Single(ws.TopCell.Persistent.OfType<IPlacedNpcGetter>());
        Assert.Equal(1000, placed.Placement!.Position.X);
    }

    [Fact]
    public void CopyOfCell_ForcesFormVersion44EvenWhenSourceIsOlder()
    {
        // Simulate a Skyrim.esm-era cell (FormVersion 40) used as the placement source.
        var sourceMod = new SkyrimMod(ModKey.FromFileName("Skyrim.esm"), SkyrimRelease.SkyrimSE);
        var sourceCell = new Cell(FormKey.Factory("01605E:Skyrim.esm"), SkyrimRelease.SkyrimSE)
        {
            FormVersion = 40,
            EditorID = "WhiterunBanneredMare",
        };
        var outMod = new SkyrimMod(ModKey.FromFileName("FF_Fv.esp"), SkyrimRelease.SkyrimSE) { IsSmallMaster = true };
        var placed = new PlacedNpc(outMod.GetNextFormKey(), SkyrimRelease.SkyrimSE);
        CellPlacer.PlaceInInterior(outMod, sourceCell.FormKey, placed, sourceCell);

        var cell = outMod.Cells.Records.SelectMany(b => b.SubBlocks).SelectMany(s => s.Cells).Single();
        Assert.Equal(44, cell.FormVersion);
