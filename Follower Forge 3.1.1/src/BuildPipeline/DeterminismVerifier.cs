...
public sealed class DeterminismVerifier(ILogger log)
{
    ...
    public Result Verify(FollowerProfile profile, EnvironmentSnapshot env,
        SpawnLocation? location = null, CatalogDb? catalog = null)
    {
        ...
        try
        {
            var ra = builder.Build(profile, env, a, location, catalog);
            var rb = builder.Build(profile, env, b, location, catalog);
