...
public static class Program
{
    ...
    private static int CmdLocations(Dictionary<string, string> opts)
    {
        ...
        if (library is null)
        {
            Console.Error.WriteLine($"No location library at {libPath} — run 'fforge locations --scan' first.");
            return 4;
        }
    ...
    private static int CmdVoiceCoverage(Dictionary<string, string> opts)
    {
        ...
        if (library is null)
        {
            Console.Error.WriteLine("No voice-coverage data — run 'fforge voice-coverage --scan' first.");
            return 4;
        }
    ...
    private static int CmdVoices(Dictionary<string, string> opts)
    {
        ...
        if (!File.Exists(dbPath))
        {
            Console.Error.WriteLine($"\nNo catalogue at {dbPath} — run 'fforge index' to cross-check your voice types.");
            return 4;
        }
    ...
    {
        var env = new VortexDiscovery(Log.Logger).Discover(opts.GetValueOrDefault("game-path"));
        var faces = new CharGenDiscovery(Log.Logger).Discover(env.GameDataPath)
        ...
        foreach (var f in faces)
        {
            ...
            var extra = f.TintDdsPath is null ? "  (no tint: grey face)" : "";
            Console.WriteLine($"  {(f.IsUsable ? "[ok]  " : "[fail]")} {Trim(f.Name, 34),-34} {verdict}{extra}");
        }
