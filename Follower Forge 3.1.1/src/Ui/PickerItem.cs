...
public sealed class FaceItem(CharGenExport export) : IPickerRow
{
    ...
    public string? Detail
    {
        get
        {
            ...
            if (SliderOnly)
                bits.Add($"shape is {Export.PresetAppearance!.SliderCount} sliders with no sculpt — "
                         + "will flatten on a follower");
    ...
    /// <summary>
    /// A preset whose shape is all RaceMenu sliders and no sculpt. Those sliders move the head's
    /// nodes on a live actor, so they cannot follow the face onto an NPC.
    /// </summary>
