...
public sealed class HubAssetPackager(ILogger log)
{
        ...
        EnvironmentSnapshot env, WriteGuard guard, string? permission, ValidationReport report)
    {
        ...
        var source = env.Manager == ModManagerKind.Mo2
            ? Mo2PathResolver.ResolveRelative(env, relPath)
            : Path.Combine(env.GameDataPath, relPath);
        if (source is null || !File.Exists(source)) return (null, false);
