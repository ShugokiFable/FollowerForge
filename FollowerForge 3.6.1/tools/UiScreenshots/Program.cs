...
internal static class Program
{
    ...
    private static void RenderTheme(string outDir, double pumpSeconds, UiTheme theme)
    {
        ...
        // Review page: the build action row, where "Copy diagnostics" sits next to Build and
        // Open output folder. Four buttons on one line is where a row starts wrapping badly.
        window.FindControl<ScrollViewer>("Page4")!.IsVisible = false;
        window.FindControl<ScrollViewer>("Page6")!.IsVisible = true;
        window.Width = 1320;
        window.Height = 900;
        Pump(2);
        Save(window, Path.Combine(outDir, $"{theme}-5-review.png"));

        // Same page at the window's MinWidth. These pages disable horizontal scrolling, so a
        // control row that does not wrap here is simply unreachable for the user.
        window.Width = 1040;
        window.Height = 700;
        Pump(2);
        Save(window, Path.Combine(outDir, $"{theme}-6-review-minwidth.png"));
