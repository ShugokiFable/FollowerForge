...
public sealed class WriteGuardTests
{
    ...
    [Fact]
    public void Allow_LetsAnExplicitUserFolderThroughAProtectedTree()
    {
        var guard = new WriteGuard();
        var staging = Path.Combine(Path.GetTempPath(), "ff_vortex_mods_" + Guid.NewGuid().ToString("N"));
        var dest = Path.Combine(staging, "MyFollower");
        guard.Protect(staging);
        Assert.True(guard.IsProtected(dest));

        guard.Allow(staging);
        Assert.False(guard.IsProtected(dest));
