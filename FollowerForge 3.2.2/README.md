...
where in Skyrim they wait. FollowerForge reads the active Vortex profile and writes a real
ESL-flagged plugin plus an installable Vortex ZIP.

## Getting started

1. Unzip anywhere and run **FollowerForge.exe**. The self-contained build includes .NET.
2. First launch indexes the active Vortex deployment. The catalogue rebuilds when the deployment
   changes or when FollowerForge's catalogue schema changes.
...
For real armor choices, FollowerForge creates a private engine starting-equipment set behind the
...
FollowerForge checks referenced face assets and reports missing paths. It does not copy another
...
Run `fforge voice-coverage --scan` once. FollowerForge then reads every installed plugin for
...
- **It uses our own script.** Every Enemy-to-Ally mod ships the same shared `KWYK_Quest`, which is
  why they all report conflicts with one another. FollowerForge ships `FF_RandomSpawn` instead,
...
FollowerForge references vanilla packages rather than authoring new ones, so this adds no
...
**This is the only feature that puts a script in your follower.** Everything else FollowerForge
...
FollowerForge reads your actual load order (from the `voice-coverage` scan) and tells you which
...
FollowerForge scans placed NPCs in the active Vortex load order and turns their cells and
...
work goes to FollowerForge's LocalAppData workspace unless another safe output path is supplied.
...
.\Publish-FollowerForge.ps1 -Version 3.1.1
```

The publish script runs tests, creates self-contained Windows app and CLI executables, performs a
12-second boot check, and writes `dist\FollowerForge-3.1.1-win-x64.zip`.
