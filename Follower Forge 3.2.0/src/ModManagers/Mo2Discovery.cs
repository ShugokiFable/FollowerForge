...
public sealed class Mo2Discovery(ILogger log)
{
    ...
    public EnvironmentSnapshot? TryDiscover(string? instanceOverride = null, string? gameRootOverride = null)
    {
        ...
        var gameRoot = gameRootOverride
            ?? GetIni(ini, "General", "gamePath")
            ?? Environment.GetEnvironmentVariable("FFORGE_GAME_PATH");
        if (string.IsNullOrWhiteSpace(gameRoot) || !Directory.Exists(Path.Combine(gameRoot, "Data")))
        {
            log.Warning("MO2 instance {Instance} has no usable gamePath", instance);
            return null;
        }
        ...
        // Stash overwrite path via a well-known first-class folder name in Mo2ModPriority;
        // Mo2PathResolver maps __mo2_overwrite__ to overwritePath by reading ini again is awkward.
        // Store overwrite as InstancePath/overwrite by convention when priority uses the token.
        log.Information(
            "Environment (MO2): instance={Instance}, profile={Profile}, {Enabled} enabled plugins, {Mods} mods",
            instance, selected, enabledCount, stagingModCount);
        return snapshot with
        {
            // Encode overwrite path for the resolver without expanding the record further:
            // Mo2PathResolver reads overwrite from InstancePath/overwrite or Settings.
            Warnings = warnings,
        };
    }
    ...
    private string? ResolveInstanceRoot(string? instanceOverride, List<string> warnings)
    {
        ...
        {
            if (File.Exists(Path.Combine(candidate, "ModOrganizer.ini")))
    ...
    private static IEnumerable<string> CandidateInstances(string? instanceOverride)
    {
        ...
        var houseCarlShim = Path.Combine(local, "houseCARL-Shim");
        if (Directory.Exists(houseCarlShim))
            yield return houseCarlShim;
    }
