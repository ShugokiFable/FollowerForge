...
/// <summary>Indexes the contents of every BSA in the game Data folder (and optional MO2 mods).</summary>
public sealed class ArchiveIndexer(ILogger log)
{
    public IEnumerable<AssetFile> Enumerate(string gameDataPath) =>
        EnumerateBsaFiles(Directory.EnumerateFiles(gameDataPath, "*.bsa", SearchOption.TopDirectoryOnly), sourceMod: null);

    /// <summary>BSAs shipped inside MO2 mod folders (not deployed into game Data).</summary>
    public IEnumerable<AssetFile> EnumerateModFolders(EnvironmentSnapshot env)
    {
        foreach (var (modName, dir) in Mo2PathResolver.EnumerateModDirsLowestFirst(env))
        {
            IEnumerable<string> bsas;
            try { bsas = Directory.EnumerateFiles(dir, "*.bsa", SearchOption.TopDirectoryOnly); }
            catch (IOException) { continue; }
            foreach (var asset in EnumerateBsaFiles(bsas, sourceMod: modName))
                yield return asset;
        }
    }

    private IEnumerable<AssetFile> EnumerateBsaFiles(IEnumerable<string> bsaFiles, string? sourceMod)
    {
        var list = bsaFiles.ToList();
        log.Information("Archive index: reading {Count} BSAs…", list.Count);
        var done = 0;
        foreach (var bsaPath in list)
        {
            ...
            if (done % 100 == 0) log.Information("  …{Done}/{Total} BSAs", done, list.Count);
        }
    }
}

/// <summary>Indexes loose files from an MO2 instance (mod priority, last wins via ReplaceAssets).</summary>
public sealed class Mo2LooseFileIndexer(ILogger log)
{
    private static readonly HashSet<string> RelevantExtensions = new(StringComparer.OrdinalIgnoreCase)
    {
        ".nif", ".dds", ".tri", ".hkx", ".fuz", ".wav", ".xwm", ".lip", ".seq",
        ".esp", ".esm", ".esl", ".bsa", ".jslot", ".json", ".xml", ".pex", ".npc",
    };

    public IEnumerable<AssetFile> Enumerate(EnvironmentSnapshot env)
    {
        long kept = 0;
        foreach (var (modName, dir) in Mo2PathResolver.EnumerateModDirsLowestFirst(env))
        {
            ...
            {
                log.Warning("Cannot enumerate MO2 mod {Mod}: {Error}", modName, ex.Message);
                continue;
            }
            ...
            foreach (var file in files)
            {
                ...
                if (rel.StartsWith("meta.ini", StringComparison.OrdinalIgnoreCase)) continue;
                kept++;
                yield return new AssetFile
                {
                    RelPath = CatalogDb.NormalizeRelPath(rel),
                    Container = AssetContainerKind.Loose,
                    ContainerName = modName,
                    SourceMod = modName,
                    Size = 0,
                };
            }
        }
        log.Information("MO2 loose-file index: kept {Kept} relevant files", kept);
    }
}
