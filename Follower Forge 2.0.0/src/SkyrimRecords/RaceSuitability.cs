...
public enum RaceClass
{
    ...
    CustomOther,
    /// <summary>Beast/undead/child/creature races: never appropriate for a follower.</summary>
    Unsuitable,
}
...
public static class RaceSuitability
{
    ...
    public static RaceOption Classify(IndexedRecord race)
    {
        ...
        if (Disqualifying.Any(bad => edid.Contains(bad, StringComparison.OrdinalIgnoreCase)))
            return new RaceOption(race.FormKey, display, RaceClass.Unsuitable, "not a follower race", plugin);

        // A race the follower can only use if the downloader installs its mod too.
        var (playable, hasHead) = ReadDetail(race);
        if (!hasHead)
            return new RaceOption(race.FormKey, display, RaceClass.Unsuitable,
                "no head parts — the face would not build", plugin);
    ...
    /// <summary>Vanilla first, then custom playable, then the rest; unsuitable races removed.</summary>
