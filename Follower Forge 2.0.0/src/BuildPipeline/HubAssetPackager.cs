...
/// writing, that they hold the right to redistribute those files — Follower Forge never infers
...
public sealed class HubAssetPackager(ILogger log)
{
        ...
        ValidationReport report)
    {
        ...
        foreach (var (shape, slot, path) in nif.AllSlots().ToList())
        {
            ...
            if (mode == HubMode.FreeHubs && kind == FaceAssetKind.SkinSupportMap)
            {
                ...
                {
                    nif.SetSlot(shape, slot, replacement);
                    changed = true;
                    retargeted++;
                    entry = entry with { CoverableByHub = HubCatalog.NazId, RetargetedTo = replacement };
                    if (!hubsUsed.Contains(naz!.Name)) hubsUsed.Add(naz.Name);
                }
            }
        ...
        string followerName, string? prefix, string? declaration, IReadOnlyList<FaceAsset> assets)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("Follower Forge copied the files below because you said you may redistribute them.");
