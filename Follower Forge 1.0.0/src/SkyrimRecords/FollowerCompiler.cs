...
public sealed class FollowerCompiler(ILogger log)
{
    ...
    /// <param name="placementWorldspace">
    /// Winning WhiterunWorld getter used to create a minimal persistent-cell override for the ACHR.
    /// When null, a stub override is constructed (used by unit tests without a live game).
    /// </param>
    public CompileResult Compile(FollowerProfile profile, IWorldspaceGetter? placementWorldspace,
        ICombatStyleGetter? combatStyleToClone = null)
    {
        ...
        var placedKey = BuildPlacement(mod, npc, profile, placementWorldspace);
        ...
        IWorldspaceGetter? placementWorldspace)
    {
        ...
        // Default exterior placement when the profile keeps the built-in Whiterun cell.
        var useDefaultExterior = pos.Cell.FormKey == VanillaForms.WhiterunWorldPersistentCell.ToString()
                                 || pos.Cell.FormKey == VanillaForms.WhiterunWorld.ToString();
        ...
        var placed = new PlacedNpc(mod.GetNextFormKey(), VanillaForms.Release)
        {
            ...
            Placement = new Placement
            {
                ...
                Rotation = new P3Float(0, 0, useDefaultExterior ? 0 : pos.AngleZDeg * MathF.PI / 180f),
            },
        };

        var worldspace = placementWorldspace is not null
            ? mod.Worldspaces.GetOrAddAsOverride(placementWorldspace)
            : new Worldspace(VanillaForms.WhiterunWorld, VanillaForms.Release);
        if (placementWorldspace is null && !mod.Worldspaces.ContainsKey(worldspace.FormKey))
            mod.Worldspaces.Add(worldspace);

        // The worldspace override is minimal (GetOrAddAsOverride yields an empty shell);
        // set its persistent cell as an override that carries only our new reference.
        worldspace.TopCell ??= new Cell(VanillaForms.WhiterunWorldPersistentCell, VanillaForms.Release);
