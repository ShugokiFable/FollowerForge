...
public sealed class FollowerBuilder(ILogger log)
{
        ...
        ValidationReport report)
    {
        ...
        if (library is null)
        {
            report.Add(ValidationSeverity.Warning, "MARRIAGE_UNKNOWN",
                "Cannot tell whether this voice can marry — run 'fforge voice-coverage --scan' once "
        ...
        DateTimeOffset buildTimeUtc)
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"Generated by Follower Forge on {buildTimeUtc:yyyy-MM-dd}.");
        sb.AppendLine();
        ...
        FollowerProfile profile, CatalogDb? catalog, ValidationReport report)
    {
        ...
        foreach (var item in profile.InventoryItems)
            Check(item, "equipment/inventory item", [IndexedRecordType.Armor, IndexedRecordType.Weapon]);
