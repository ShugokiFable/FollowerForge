...
public static class ActiveProfileDetector
{
        ...
        string profilesPath, string runtimePluginsTxt, List<string> warnings)
    {
        ...
        // Sets identical but line order differs: same enabled plugins, unambiguous identification.
        // (Actual load order always comes from loadorder.txt, so ordering here is not load-bearing.)
        if (best is not null && bestScore >= 0.9999)
            return (best, "same enabled plugin set as runtime (order differs)");
