...
public static class PluginLists
{
    ...
    /// Order is lowest priority first; the last enabled mod wins conflicts (MO2 rule).
    /// Returns only enabled mod folder names, still lowest→highest.
    /// </summary>
    public static IReadOnlyList<string> ParseMo2ModList(string path)
    {
        ...
        // UNVERIFIED: consumed as "last entry wins". MO2's UI shows the bottom mod winning, but
        // whether modlist.txt stores that order or its reverse has not been confirmed against a
        // real instance. If MO2 users report the wrong file winning a conflict, reverse here.
        return result;
    }
}
