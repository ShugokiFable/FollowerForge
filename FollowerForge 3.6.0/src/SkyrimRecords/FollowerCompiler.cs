...
public sealed class FollowerCompiler(ILogger log)
{
        ...
        ICombatStyleGetter? combatStyleToClone)
    {
        ...
        // Ammo is carried like anything else; it just also goes in the outfit above.
        var carried = profile.InventoryItems.Concat(profile.Ammo).ToList();
        if (carried.Count > 0)
        {
            npc.Items ??= [];
            foreach (var item in carried)
                npc.Items.Add(new ContainerEntry
                {
                    ...
                    {
                        Item = FormKey.Factory(item.FormKey).ToLink<IItemGetter>(),
                        // A stack of 100 arrows is one entry with Count 100, not 100 entries.
        ...
        Npc npc, AppearanceSpec appearance, string prefix, SkyrimMod mod)
    {
        ...
        // Pin the complexion the preset was made with. The race default is only right by luck,
        // and when it is wrong the exported head and the body meet at a visible neck seam.
        if (appearance.HeadTextureSet is { } headTexture)
            npc.HeadTexture.SetTo(FormKey.Factory(headTexture.FormKey));
    }
    ...
    /// <summary>
    /// Builds the placed ACHR. Library locations are preferred. Manual
    /// <see cref="PlacementSpec.Cell"/> placements use that exact cell (and optional worldspace).
    /// Only the no-cell default hardcodes WhiterunWorld's persistent cell (01A270).
    /// </summary>
        ...
        SpawnLocation? location, ICellGetter? cellSource)
    {
        ...
        // Manual cell + coordinates: honor the specified cell. Never silently substitute Whiterun.
        if (pos.Cell is { } cellRef)
        {
            var cellKey = FormKey.Factory(cellRef.FormKey);
            placed.Placement = new Placement
            {
            ...
            if (pos.Worldspace is { } worldRef)
            {
                CellPlacer.PlaceInWorldspace(mod, FormKey.Factory(worldRef.FormKey), cellKey, placed, cellSource);
                log.Information("Placed {Name} at manual exterior cell {Cell} in {World}",
                    profile.Name, cellKey, worldRef.FormKey);
                return placed.FormKey;
            }

            // Back-compat: bare Whiterun persistent cell without Worldspace still means WhiterunWorld.
            if (cellKey.Equals(VanillaForms.WhiterunWorldPersistentCell))
            {
                CellPlacer.PlaceInWorldspace(mod, VanillaForms.WhiterunWorld, cellKey, placed, cellSource);
                log.Information("Placed {Name} at Whiterun exterior (manual cell)", profile.Name);
                return placed.FormKey;
            }

            // Interior (or any non-Whiterun cell without an explicit worldspace): place in that CELL.
            // Exterior worldspace top cells outside Whiterun must set Placement.Worldspace.
            var treatAsInterior = cellSource is null
                || cellSource.Flags.HasFlag(Cell.Flag.IsInteriorCell);
            if (!treatAsInterior)
            {
                log.Warning(
                    "Manual exterior cell {Cell} has no Placement.Worldspace; placing as a cell override. " +
                    "Set Worldspace for worldspace-persistent exteriors outside Whiterun.",
                    cellKey);
            }
            CellPlacer.PlaceInInterior(mod, cellKey, placed, cellSource);
            log.Information("Placed {Name} at manual cell {Cell}", profile.Name, cellKey);
            return placed.FormKey;
        }

        // No library entry and no manual cell: built-in Whiterun exterior spot.
        placed.Placement = new Placement
        {
            Position = new P3Float(
                VanillaForms.WhiterunDefaultPos.X,
                VanillaForms.WhiterunDefaultPos.Y,
                VanillaForms.WhiterunDefaultPos.Z),
            Rotation = new P3Float(0, 0, pos.AngleZDeg * MathF.PI / 180f),
        };
