...
public sealed class FollowerCompilerTests
{
    ...
    {
        var result = new FollowerCompiler(Log).Compile(SampleProfile(), location: null);
        ...
        var placed = Assert.Single(ws.TopCell!.Persistent);
        Assert.NotEqual(0u, (uint)placed.MajorRecordFlagsRaw & 0x00000400u);
    }
