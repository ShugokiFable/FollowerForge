...
public sealed class RaceSuitabilityTests
{
    ...
    {
        // These are now Creature rather than Unsuitable — real mods ship creature followers, so
        // they can be chosen deliberately. What must never change is that they stay hidden
        // unless asked for.
        Assert.Equal(RaceClass.Creature, RaceSuitability.Classify(Race(editorId)).Class);
        Assert.Empty(RaceSuitability.Offer([Race(editorId)]));
    }
