...
$stage = Join-Path $dist "Follower Forge $Version"
$zip = Join-Path $dist "FollowerForge-$Version-win-x64.zip"

Write-Host "== Follower Forge $Version - publish ==" -ForegroundColor Cyan

foreach ($name in 'Follower Forge', 'FollowerForge.Ui', 'FollowerForge.Cli') {
...
$exe = Join-Path $stage 'Follower Forge.exe'
...
Write-Host "  exe    : Follower Forge.exe  (self-contained - no .NET needed)"
exit 0
